...
function ListingViewModal({ listing, onClose }: { listing: ListingDetail; onClose: () => void }) {
  ...
  const relationDetails = ['propertyCategory', 'houseType', 'landType', 'propertyType', 'broker', 'owner', 'region', 'district', 'ward'].filter((key) => listing[key])
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="admin-modal listing-view-modal"><div className="modal-heading"><div><span>Property details</span><h2>{listing.title || 'Untitled property'}</h2></div><button onClick={onClose} title="Close"><i className="bi bi-x-lg" /></button></div><div className="listing-view-content"><div className="listing-detail-grid">{scalarDetails.map(([key, value]) => <div key={key}><small>{prettyLabel(key)}</small><strong>{formatDetailValue(value, key)}</strong></div>)}{relationDetails.map((key) => <div key={key}><small>{prettyLabel(key)}</small><strong>{formatDetailValue(listing[key], key)}</strong></div>)}</div><MediaSection title="Images" icon="images" empty={!images.length}>{images.map((image) => <a className="listing-image-link" href={getUploadUrl(image.url)} target="_blank" rel="noreferrer" key={image.id || image.url}><img src={getUploadUrl(image.url)} alt="Property" /></a>)}</MediaSection><MediaSection title="Videos" icon="camera-video" empty={!videos.length}>{videos.map((video) => <video className="listing-video" controls preload="metadata" src={getUploadUrl(video.url)} key={video.id || video.url} />)}</MediaSection><MediaSection title="Documents" icon="paperclip" empty={!documents.length}>{documents.map((document) => <a className="listing-document" href={getUploadUrl(document.url)} target="_blank" rel="noreferrer" key={document.id || document.url}><i className="bi bi-file-earmark-text" /> {document.name || 'Document'}</a>)}</MediaSection><MediaSection title="Features" icon="check2-circle" empty={!listing.features?.length}>{(listing.features || []).map((feature: any) => <span className="listing-feature" key={feature.id || feature.name}>{feature.name || feature}</span>)}</MediaSection></div></div></div>
}
