import { createContext, useContext, useState, type ReactNode } from 'react'

export type Language = 'sw' | 'en'

const translations: Record<string, string> = {
  'Register Estate': 'Sajili Mali',
  'Register an Estate': 'Sajili Mali Isiyohamishika',
  'Pick the type of property your customer wants to sell.': 'Chagua aina ya mali ambayo mteja wako anataka kuuza.',
  'List property available for sale': 'Orodhesha mali inayouzwa',
  'Unable to load property categories.': 'Imeshindikana kupakia aina za mali.',
  'Loading property categories...': 'Inapakia aina za mali...',
  'House for Sale': 'Nyumba ya Kuuza',
  'Land for Sale': 'Ardhi ya Kuuza',
  'Commercial Area for Sale': 'Eneo la Biashara la Kuuza',
  'Commercial Area': 'Eneo la Biashara',
  'Property details': 'Maelezo ya mali',
  'Provide details about the house.': 'Weka maelezo ya nyumba.',
  'Provide details about the land.': 'Weka maelezo ya ardhi.',
  'Provide details about the property.': 'Weka maelezo ya mali.',
  'Property title': 'Jina la mali',
  'Sale price (TZS)': 'Bei ya kuuza (TZS)',
  'Size unit': 'Kipimo cha ukubwa',
  'Size': 'Ukubwa',
  'House type': 'Aina ya nyumba',
  'Land type': 'Aina ya ardhi',
  'Commercial property type': 'Aina ya mali ya biashara',
  'Features & amenities': 'Vipengele na huduma',
  'Select the features this property has.': 'Chagua vipengele mali hii inavyo.',
  'Broker Info': 'Taarifa za wakala',
  'Owner Info': 'Taarifa za mmiliki',
  'Who holds title to this property.': 'Mwenye hati ya umiliki wa mali hii.',
  'Full name': 'Jina kamili',
  'Search an address or place...': 'Tafuta anwani au mahali...',
  'Search': 'Tafuta',
  'My location': 'Mahali nilipo',
  'Loading map...': 'Inapakia ramani...',
  'Location set:': 'Mahali pamewekwa:',
  'No location set. Click the map, search, or use your location.': 'Hakuna mahali palipowekwa. Bofya ramani, tafuta, au tumia mahali ulipo.',
  'Location & Images': 'Mahali na Picha',
  'Region, GPS and photos': 'Mkoa, GPS na picha',
  'Location': 'Mahali',
  'Map': 'Ramani',
  'Description': 'Maelezo',
  'Property images': 'Picha za mali',
  'Add images': 'Ongeza picha',
  'Verified documents': 'Nyaraka zilizothibitishwa',
  'Attach documents': 'Ambatisha nyaraka',
  'Review': 'Kagua',
  'Details': 'Maelezo',
  'Review the details below, then submit — or go back to edit.': 'Kagua maelezo hapa chini, kisha tuma au rudi kuyahariri.',
  'Category': 'Aina',
  'Status': 'Hali',
  'Broker': 'Wakala',
  'Owner': 'Mmiliki',
  'Region / District': 'Mkoa / Wilaya',
  'Ward / Exact location': 'Kata / Mahali halisi',
  'Images': 'Picha',
  'Video & Docs': 'Video na Nyaraka',
  'Video & Documents': 'Video na Nyaraka',
  'Add the property video and verified documents.': 'Ongeza video ya mali na nyaraka zilizothibitishwa.',
  'Add photos for this property.': 'Ongeza picha za mali hii.',
  'Broker & Owner Info': 'Taarifa za Wakala na Mmiliki',
  'Broker & Owner': 'Wakala na Mmiliki',
  'Who is submitting this listing?': 'Nani anayewasilisha tangazo hili?',
  'Wakala (Broker)': 'Wakala',
  'Mmiliki (Owner)': 'Mmiliki',
  'Documents': 'Nyaraka',
  'Features': 'Vipengele',
  'Cancel': 'Ghairi',
  'Back': 'Rudi',
  'Continue': 'Endelea',
  'Save': 'Hifadhi',
  'Estate submitted': 'Mali imetumwa',
  'The listing has been saved and is ready for review.': 'Orodha imehifadhiwa na iko tayari kukaguliwa.',
  'Register another estate': 'Sajili mali nyingine',
  'Please complete the property title, sale price, and property type.': 'Jaza jina la mali, bei ya kuuza, na aina ya mali.',
  'Broker and owner names and phone numbers are required.': 'Majina na namba za simu za wakala na mmiliki zinahitajika.',
  'Owner name and phone number are required.': 'Jina na namba ya simu ya mmiliki zinahitajika.',
  'Failed to submit property. Please try again.': 'Imeshindwa kusajili mali. Tafadhali hakikisha una mtandao kisha jaribu tena.',
  'Please fill this field': 'Tafadhali jaza sehemu hii',
  'Property title is required': 'Jina la mali linahitajika',
  'Sale price is required': 'Bei ya kuuza inahitajika',
  'Size unit is required': 'Kipimo cha ukubwa kinahitajika',
  'Full name is required': 'Jina kamili linahitajika',
  'Phone number is required': 'Namba ya simu inahitajika',
  'Please select a house type': 'Tafadhali chagua aina ya nyumba',
  'Please select a land type': 'Tafadhali chagua aina ya kiwanja',
  'Please select a property type': 'Tafadhali chagua aina ya mali',
  'Add at least one image': 'Ongeza angalau picha moja',
  'A video is required': 'Video inahitajika',
  'Uploading video': 'Inapakia video',
  'Finishing up...': 'Inakamilisha...',
  'Attach at least one document': 'Ambatisha angalau nyaraka moja',
  'Please select a region': 'Tafadhali chagua mkoa',
  'Please select a district': 'Tafadhali chagua wilaya',
  'Please select a ward': 'Tafadhali chagua kata',
  'Exact location is required': 'Mahali halisi panahitajika',
  'Pin the location on the map': 'Weka alama ya mahali kwenye ramani',
  'Region': 'Mkoa',
  'District': 'Wilaya',
  'Ward / Area': 'Kata / Eneo',
  'Exact location': 'Mahali halisi',
  'Choose a region first': 'Chagua mkoa kwanza',
  'Choose a district first': 'Chagua wilaya kwanza',
  'Select a district...': 'Chagua wilaya...',
  'Select a ward...': 'Chagua kata...',
  'Select a region...': 'Chagua mkoa...',
  'WhatsApp phone number (+255)': 'Namba ya WhatsApp (+255)',
  'NIDA (optional)': 'NIDA (si lazima)',
  'TIN (optional)': 'TIN (si lazima)',
  'Bedrooms': 'Vyumba vya kulala',
  'Bathrooms': 'Vyumba vya kuoga(Bathrooms)',
  'English': 'Kiingereza',
  'Kiswahili': 'Kiswahili',
  'Geolocation is not supported by your browser': 'Kivinjari chako hakiwezi kutambua mahali ulipo',
  'Unable to retrieve your location': 'Imeshindikana kupata mahali ulipo',
  'Location permission denied. Allow location access in your browser, then try again.': 'Ruhusa ya mahali imekataliwa. Ruhusu kivinjari kutumia mahali, kisha jaribu tena.',
  'Location access is blocked. Enable location in your browser settings, refresh the page, and try again. You can also click the map to pick a location manually.': 'Ruhusa ya mahali imezuiwa. Washa mahali kwenye mipangilio ya kivinjari, refresh ukurasa, kisha ujaribu tena. Unaweza pia kubonyeza ramani kuchagua mahali kwa mikono.',
  'Your location is unavailable right now. Move to an open area or pick the spot on the map.': 'Mahali ulipo hapatikani kwa sasa. Nenda eneo wazi au chagua mahali kwenye ramani.',
  'Getting your location timed out. Please try again or pick the spot on the map.': 'Muda wa kupata mahali umeisha. Jaribu tena au chagua mahali kwenye ramani.',
  'Built from the category, sub-type, size, rooms, amenities, location and price on this form. Edit it if you want to.': 'Maelezo yametokana na aina, kipimo, vyumba, huduma, mahali na bei ya mali hii. Yahariri ukitaka.',
  'Title deed, survey plan, sale agreement — PDF or a photo, up to 15 MB each. They upload when the record is saved, and more can be added later.': 'Hati ya umiliki, mpango wa upimaji, makubaliano ya mauzo, PDF au picha, hadi MB 15 kila moja. Zitapakiwa wakati rekodi inahifadhiwa.',
    'Video must be less than 30 MB.': 'Video lazima iwe chini ya MB 30.',
    'Please select a video file.': 'Tafadhali chagua faili la video.',
    'Video upload failed. Please try again.': 'Upakiaji wa video umeshindikana. Tafadhali jaribu tena.',
    'Property video': 'Video ya mali',
    'Video uploaded': 'Video imepakiwa',
    'Add one video': 'Ongeza video moja',
  'Write a feature...': 'Andika kipengele...',
  'Add': 'Ongeza',
  'Remove': 'Ondoa',
    'Video upload progress': 'Maendeleo ya upakiaji wa video',
  'Road Access': 'Upatikanaji wa barabara',
  'Electricity': 'Umeme',
  'Borehole':'Kisima',
  'Road Frontage':'Eneo la mbele la barabara',
  'Water Supply': 'Maji',
  'Sewerage': 'Mazao',
  'Ready for Development': 'tayari kwa kuendelezwa',
  'Closer to CBD':'Maji yanapatikana',
  'Tiled Floor':'vigae',
  'Agricultural':'Kilimo',
  'Surveyed':'Pamepimwa',
  'Fitted Kitchen':'jiko la ndani',
  'Commercial':'Biashara',
  'Mixed used':'Mengine',
  'Mixed':'Mchanganyiko',
  'Mixed Use':'Mengine',
  'Residential':'Makazi',
  'Tarmac Road Access':'karibu na lami',
  'Parking': 'Maegesho',
  'Security': 'Ulinzi',
  'Fence': 'Uzio',
  'Furnished': 'Imewekewa samani',
  'Garden': 'Bustani',
  'Swimming Pool': 'Swimming Pool',
  'Other': 'Nyingine',
  'Commercial Land & Building': 'Ardhi na Jengo la Biashara',
  'Commercial Building': 'Jengo la Biashara',
  'Commercial Land': 'Ardhi ya Biashara',
  'Commercial Zoning': 'Ukanda wa Biashara',
  'Highway Access': 'Upatikanaji wa Barabara Kuu',
  'Buildings and open land plots show different amenities.': 'Majengo na viwanja wazi vina huduma tofauti.',
  'Not set': 'Haijawekwa',
  'Unassigned': 'Hajapangiwa',
  'Not registered': 'Hajasajiliwa',
  'None': 'Hakuna',
  'Condominium': 'Condominium',
  'Duplex': 'Duplex',
  'Single Room': 'Chumba',
  'Standalone': 'Standalone',
  'Dining Room': 'Chumba cha Chakula(Dining Room)',
  'Sitting Room': 'Sebule(Sitting Room)',
  'En-suite Bedrooms': 'master bedroom',
  'Balcony': 'Baraza',
  'Store Room': 'Stoo',
  'Servant Quarter': 'Servant Quarter',
  'Generator': 'Jenereta',
  'Air Conditioning': 'AC',
  'Outside Kitchen': 'Jiko inje',
  'CCTV': 'CCTV',
  'Elevator': 'Lifti',
  'Fiber Internet': 'Intaneti ya Fiber',
  'Security Guards': 'Walinzi',
  'Wheelchair Accessible': 'Panafikika',
  'Fire Safety System': 'Mfumo wa Usalama wa Moto',
  'Conference Room': 'Ukumbi',
  'Loading Bay': 'Eneo la Kupakua Mizigo',
  'Corner Plot': 'Kiwanja cha Kona',
  'Office Building': 'Jengo la Ofisi',
  'Retail Shop': 'Duka la Rejareja',
  'Showroom': 'showroom',
  'Warehouse': 'Ghala',
  'Factory': 'Kiwanda',
  'Garage / Workshop': 'Gereji',
  'Hotel': 'Hoteli',
  'Guest House': 'Nyumba ya Wageni',
  'Restaurant Space': 'Eneo la Mgahawa',
  'Bar / Lounge': 'Baa / Loji',
  'Commercial Plot': 'Eneo la Biashara',
  'Industrial Plot': 'Eneo la Kiwanda',
  'Yard': 'Uwanja',
  'Car Wash': 'Car Wash',
  'Petrol Station': 'Stesheni ya Mafuta',
  'Vehicle Parking Lot': 'Eneo la Maegesho ya Magari',
  'sqm': 'sqm',
  'acre': 'acre',
  'plot': 'plot',
  'metre': 'metre',
  'feet': 'feet',
}

function capitalizeFirstLetter(text: string) {
  return text.replace(/^(\s*)(\S)/u, (_, whitespace: string, firstLetter: string) => `${whitespace}${firstLetter.toLocaleUpperCase()}`)
}

interface LanguageContextValue {
  language: Language
  setLanguage: (language: Language) => void
  tr: (text: string) => string
}

const LanguageContext = createContext<LanguageContextValue | null>(null)

export function LanguageProvider({ children }: { children: ReactNode }) {
  const [language, setLanguage] = useState<Language>('sw')
  const tr = (text: string) => capitalizeFirstLetter(language === 'sw' ? (translations[text] || text) : text)
  return <LanguageContext.Provider value={{ language, setLanguage, tr }}>{children}</LanguageContext.Provider>
}

export function useLanguage() {
  const context = useContext(LanguageContext)
  if (!context) throw new Error('useLanguage must be used within LanguageProvider')
  return context
}
